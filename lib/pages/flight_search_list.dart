...
class _FlightListState extends State<FlightList> {
    ...
    Future.delayed(Duration.zero, () {
      setState(() {
        ...
        if (data != null) {
          ...
          _pdt = _departdate.year.toString() +
              "-" +
              month_str +
              "-" +
              day_str +
              "T00:00:00";
          _pat = _departdate.year.toString() +
              "-" +
              month_str +
              "-" +
              day_str +
      ...
      _populateNewsArticles(_adult, _child, _infant, _type, _origin,
          _destination, _pdt, _pat, _bookingclass);
      _getsearchvalue(_adult, _child, _infant, _type, _origin, _destination,
          _pdt, _pat, _bookingclass);
    });
    super.initState();
  ...
  void _populateNewsArticles(int adult, int child, int infant, int type,
      String origin, String destination, String pdt, String pat, int cls) {
    allflightlist(
            adult, child, infant, type, origin, destination, pdt, pat, cls)
        .then((flightresults) => setState(() => {
  ...
  void _getsearchvalue(int adult, int child, int infant, int type,
      String origin, String destination, String pdt, String pat, int cls) {
    getflightinfo(
            adult, child, infant, type, origin, destination, pdt, pat, cls)
        .then((flightinfo) => setState(() =>
            {_traceId = flightinfo.TraceId, _srvdType = flightinfo.SrdvType}));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: SafeArea(
          ...
          if (_flightResults.isEmpty) LinearProgressIndicator(),
          ListView.builder(
              ...
              children: <Widget>[
                Row(
                  children: [
                    Image(
                      image: NetworkImage(
                          'https://pbs.twimg.com/profile_images/1263869715740389376/ev8EtB81.jpg'),
                      height: 40,
                      width: 40,
                    )
                  ],
                        ...
                        SizedBox(
                            ...
                            child: RaisedButton(
                              ...
                              onPressed: () {
                                ...
                                // Navigator.pushNamed(context, '/flight_book');
                                Navigator.push(
                                    context,
                                    MaterialPageRoute(
                                        builder: (context) => FlightBook(
                                            flight: _flightResults,
                                            index: main_index)));
                              },
                              shape: RoundedRectangleBorder(
                                borderRadius: BorderRadius.circular(4),
